/**
 * Hilo de chat de un caso. Cualquier usuario autenticado puede ver y
 * postear (sujeto a moderación + reputación + shadow). Long-press para
 * reportar/silenciar.
 */
...
export default function ChatCaso() {
  ...
  const safeSlug = slug ?? "";
  const me = useMe();
  const query = useChatCaso(safeSlug || null);
  const send = useEnviarCaso(safeSlug);
  const reportar = useReportarMensaje({ casoSlug: safeSlug });
  ...
  const submit = async () => {
    if (text.trim().length < 2 || !safeSlug) return;
    try {
  ...
  return (
    <Screen edges={["top"]} padded={false}>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <H2 style={{ fontSize: 20 }} numberOfLines={1}>
            Hilo del caso
          </H2>
          <Text style={{ fontSize: 11, color: colors.muted, marginTop: 2 }}>
            Solo conversación útil para este caso.
          </Text>
        </View>
      </View>
      ...
      {query.isPending ? (
        ...
        <EmptyState
          ...
          description="Comparte una pista o pregunta. Mantén pulsado un mensaje para reportar."
        />
      ) : (
        <FlatList
          ref={listRef}
      ...
      >
        <Input
          placeholder="Comparte una pista o pregunta..."
          multiline
          value={text}
